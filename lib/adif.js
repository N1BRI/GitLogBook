...
function parseAdif(input) {
  ...
  return body
    ...
    .map(parseRecord)
    .filter(Boolean)
    .map((qso) => normalizeParsedQso({ ...qso, id: qso.id || crypto.randomUUID() }));
}
...
function normalizeParsedQso(qso) {
  return {
    ...qso,
    band: normalizeBand(qso.band),
    band_rx: normalizeBand(qso.band_rx)
  };
}
...
function writeRecord(qso) {
  ...
  const extraKeys = Object.keys(qso)
    .filter((key) => !known.has(key) && qso[key] !== undefined && qso[key] !== null && String(qso[key]).trim() !== "")
    .sort();
